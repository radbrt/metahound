...
from metahound.cli_functions import push_fn


def test_push_fn_success(backend_with_data, monkeypatch):
    """push_fn POSTs the payload and prints success."""
    monkeypatch.setenv("METAHOUND_BACKEND_URI", "sqlite:///:memory:")

    mock_response = MagicMock()
    mock_response.json.return_value = {"ingest_event_id": 1, "sources_processed": 1}
    mock_response.raise_for_status.return_value = None

    with patch("metahound.cli_functions.GenericBackendHandler", return_value=backend_with_data):
...
def test_push_fn_no_token_raises(monkeypatch):
    ...
    monkeypatch.delenv("METAHOUND_API_TOKEN", raising=False)
...
def test_push_fn_no_url_raises(monkeypatch):
    ...
    monkeypatch.setenv("METAHOUND_API_TOKEN", "some-token")
    monkeypatch.delenv("METAHOUND_API_URL", raising=False)
